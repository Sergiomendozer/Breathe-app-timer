...
const holdAfterExhaleCircle = document.querySelector('.hold-after-exhale');
...
let HOLD_TIME = 4;
let EXHALE_TIME = 5;
let HOLD_AFTER_EXHALE_TIME = 4; // Changed from 2 to 4
let TOTAL_CYCLE_TIME = INHALE_TIME + HOLD_TIME + EXHALE_TIME + HOLD_AFTER_EXHALE_TIME;
...
function updateCircles(cycleTime) {
  ...
  if (cycleTime < INHALE_TIME) {
  ...
  } else if (cycleTime < INHALE_TIME + HOLD_TIME + EXHALE_TIME) {
    inhaleCircle.classList.add('active');
    ...
    inhaleCircle.classList.add('active');
    holdCircle.classList.add('active');
    exhaleCircle.classList.add('active');
    updateCircle(holdAfterExhaleCircle, cycleTime - INHALE_TIME - HOLD_TIME - EXHALE_TIME, HOLD_AFTER_EXHALE_TIME);
  }
}
...
function resetCircles() {
  [inhaleCircle, holdCircle, exhaleCircle, holdAfterExhaleCircle].forEach(circle => {
...
function updateDurationDisplay() {
  ...
  holdAfterExhaleCircle.querySelector('.timer-duration').textContent = `${HOLD_AFTER_EXHALE_TIME.toFixed(1)}s`;
}
...
function updateDuration(target, increment) {
  ...
  switch (target) {
    ...
    case 'hold-after-exhale':
      HOLD_AFTER_EXHALE_TIME = Math.min(Math.max(HOLD_AFTER_EXHALE_TIME + increment, minDuration), maxDuration);
      break;
  }

  TOTAL_CYCLE_TIME = INHALE_TIME + HOLD_TIME + EXHALE_TIME + HOLD_AFTER_EXHALE_TIME;
  updateDurationDisplay();
}
